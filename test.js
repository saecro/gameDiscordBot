...
// Step 1: Load the existing JSON data from the file
const existingColors = JSON.parse(fs.readFileSync('output2.json', 'utf-8'));

// Step 2: Named colors array (from the list I provided)
const namedColors = [
...
console.log(existingColors.length * namedColors.length)
namedColors.forEach(color => {
    const colorExists = existingColors.some(existingColor => {
        return (
            existingColor.hexCode.toLowerCase() === color.hex.replace('#', '').toLowerCase() ||
            existingColor.bestName.toLowerCase() === color.name.toLowerCase()
        );
    });

    if (!colorExists) {
        console.log(`Missing color: ${color.name} with hex ${color.hex}`);
    }
});
